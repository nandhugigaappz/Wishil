...
public class PartnerChangePasswordActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dialogProgress = new DialogProgress(PartnerChangePasswordActivity.this);
    ...
    private void changePassword() {
        dialogProgress.show();
        retrofit.create(PartnerChangePasswordAPI.class).post(
                userID,
                passwordOne)
                .enqueue(new Callback<PartnerChangePasswordResponse>() {
                    ...
                    public void onResponse(Call<PartnerChangePasswordResponse> call, Response<PartnerChangePasswordResponse> response) {
                        if (response.isSuccessful()) {
                            ...
                            Toast.makeText(PartnerChangePasswordActivity.this,message,Toast.LENGTH_SHORT).show();
                            finish();
                        } else {
                            Toast.makeText(PartnerChangePasswordActivity.this, "Failed", Toast.LENGTH_SHORT).show();
                            dialogProgress.dismiss();
                        }
                    }
    ...
    private boolean validatePassword() {
        if (passwordOne.equals("")) {
            Toast.makeText(PartnerChangePasswordActivity.this, "Please enter your password.", Toast.LENGTH_SHORT).show();
            return false;
        } else {
            ArrayList<PasswordValidation> arrayList = Utils.validatePassword(passwordOne);
            if (passwordOne.contains(" ")) {
                Toast.makeText(PartnerChangePasswordActivity.this, "Please enter a Password without space", Toast.LENGTH_SHORT).show();
                return false;
            }
           /* if (arrayList.get(0) != PasswordValidation.PASSWORD_OK) {
            ...
            if (!passwordOne.equals(passwordTwo)) {
                Toast.makeText(PartnerChangePasswordActivity.this, "Password Mismatch", Toast.LENGTH_SHORT).show();
                return false;
            }
            return true;
        }
