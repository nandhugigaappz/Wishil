...
public class SchoolListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    List<SubCatSchoolListData> list;
    public ItemClickAdapterListener onClickListener;
    boolean loadMore;

    public SchoolListAdapter(boolean loadMore, List<SubCatSchoolListData> list, Context context, ItemClickAdapterListener onClickListener) {
        this.list = list;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof ListHolder) {
            ...
            Glide.with(vh1.thumbnail.getContext()).load(APILinks.IMAGE_LINK + list.get(position).getSchoolImg())
                    .crossFade()
                    .thumbnail(0.5f)
                    .diskCacheStrategy(DiskCacheStrategy.ALL)
                    .into(vh1.thumbnail);
            vh1.tvName.setText(list.get(position).getSchoolName());
            String establishedYear = "Established Year: <font size='5' color='black'><b>" + list.get(position).getSchoolYear() + "</b></font> ";
            vh1.tvYear.setText(Html.fromHtml(establishedYear));
            vh1.tvLocation.setText("");
            ...
            vh1.tvEnq.setOnClickListener(new View.OnClickListener() {
                @Override
